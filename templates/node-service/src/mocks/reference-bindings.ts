...
export function makeModel(opts: ReferenceBindingOptions): ModelInterface {
  const persona = opts.persona ?? "node-service";
  return {
    descriptor: {
      modelId: "mock-slm",
      contextWindow: 4096,
      locality: "self-hosted" as const,
...
export function makePlanning(): PlanningInterface {
  return {
    async compose(goals, context) {
      ...
      const first = goals[0];
      return {
        planId: "plan-node-smoke",
        steps: [
          {
            stepId: "step-1",
            goalId: first?.goalId ?? "goal-node",
            action: "Respond to subject",
            dependsOn: [],
            status: "active",
          },
        ],
        rationale: "node-service reference planning stub",
      };
    },
  ...
  const corpus: KnowledgePassage[] = [
    {
      sourceId,
      citation: `${sourceId}#1`,
      content: "Reference knowledge passage for node-service smoke.",
      score: 1,
      asOf: "2024-06-01",
    },
  ];
