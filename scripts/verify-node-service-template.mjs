/**
 * Verify templates/node-service: copy → install → typecheck → smoke.
 *
 * Usage (repo root):
 *   node scripts/verify-node-service-template.mjs
 *   pnpm integration:node-service:verify
 */
...
export const NODE_SERVICE_TEMPLATE_ROOT = path.join(
  REPO_ROOT,
  "templates",
  "node-service",
);
...
export const OBLIGATIONS = Object.freeze({
  TEMPLATE_MISSING: "integration_templates.node_service.template_missing",
  WORKSPACE_PROTOCOL: "integration_templates.node_service.workspace_protocol",
  PACK_FAILED: "integration_templates.node_service.pack_failed",
  INSTALL_FAILED: "integration_templates.node_service.install_failed",
  TYPECHECK_FAILED: "integration_templates.node_service.typecheck_failed",
  SMOKE_FAILED: "integration_templates.node_service.smoke_failed",
});

function emit(event) {
  process.stdout.write(
    `${JSON.stringify({ event: "integration_templates.node_service.verify", ...event })}\n`,
  );
}

export function validateNodeServicePackageJson(pkg) {
  const violations = [];
...
export function wireNodeServiceConsumerDeps(consumerDir, tarballs) {
...
export function runNodeServiceTemplateVerify(opts = {}) {
  const subjectId = opts.subjectId ?? "node-service-verify";
  const deviceId = opts.deviceId ?? "ci";
  const emitEvents = opts.emitEvents !== false;
  const templateRoot = opts.templateRoot ?? NODE_SERVICE_TEMPLATE_ROOT;
  const workRoot =
    opts.workRoot ?? mkdtempSync(path.join(tmpdir(), "sutra-node-service-verify-"));
  ...
  if (!existsSync(templateRoot)) {
    return {
      ...
      combined: `NODE_SERVICE_VERIFY_FAILED (template): missing ${templateRoot}`,
    };
  }

  const pkgCheck = validateNodeServicePackageJson(
  ...
  if (pkgCheck.status !== 0) {
    ...
    return {
      ...
      combined: `NODE_SERVICE_VERIFY_FAILED (package):\n${detail}`,
    };
  }
  ...
  if (!opts.skipBuild) {
    ...
    try {
      ...
      return {
        ...
        combined: `NODE_SERVICE_VERIFY_FAILED (build): ${detail}`,
      };
    }
  }
  ...
  if (!tarballs) {
    ...
    if (packed.violations.length > 0) {
      ...
      return {
        ...
        combined: `NODE_SERVICE_VERIFY_FAILED (pack):\n${combined}`,
      };
    }
  ...
  wireNodeServiceConsumerDeps(consumerDir, tarballs);
  ...
  if (install.status !== 0) {
    ...
    if (emitEvents) emit({ outcome: "fail", subjectId, deviceId, phase: "install" });
    if (cleanup) rmSync(workRoot, { recursive: true, force: true });
    return {
      status: 1,
      phase: "install",
      violations: [{ obligation: OBLIGATIONS.INSTALL_FAILED, detail }],
      combined: `NODE_SERVICE_VERIFY_FAILED (install): ${detail}`,
    };
  }
  ...
  if (typecheck.status !== 0) {
    ...
    if (emitEvents) emit({ outcome: "fail", subjectId, deviceId, phase: "typecheck" });
    if (cleanup) rmSync(workRoot, { recursive: true, force: true });
    return {
      status: 1,
      phase: "typecheck",
      violations: [{ obligation: OBLIGATIONS.TYPECHECK_FAILED, detail }],
      combined: `NODE_SERVICE_VERIFY_FAILED (typecheck): ${detail}`,
    };
  }
  ...
  if (smoke.status !== 0) {
    ...
    if (emitEvents) emit({ outcome: "fail", subjectId, deviceId, phase: "smoke" });
    if (cleanup) rmSync(workRoot, { recursive: true, force: true });
    return {
      status: 1,
      phase: "smoke",
      violations: [{ obligation: OBLIGATIONS.SMOKE_FAILED, detail }],
      combined: `NODE_SERVICE_VERIFY_FAILED (smoke): ${detail}`,
    };
  }
  ...
  if (!String(smoke.stdout).includes("smoke OK")) {
    ...
    return {
      ...
      combined: "NODE_SERVICE_VERIFY_FAILED (smoke): missing success marker",
    };
  }
  ...
  return {
    status: 0,
    phase: "smoke",
    combined: "OK: node-service template installed, typechecked, and passed smoke",
  };
}

function main() {
  const result = runNodeServiceTemplateVerify();
  if (result.status !== 0) {
