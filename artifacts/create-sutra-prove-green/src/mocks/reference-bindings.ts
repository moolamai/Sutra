...
export function makeModel(opts: ReferenceBindingOptions): ModelInterface {
  const persona = opts.persona ?? "companion";
  const descriptor = {
    modelId: "mock-slm",
    contextWindow: 4096,
    ...
    async compose(goals, context) {
      ...
      return {
        ...
        steps: [
          {
            stepId: "step-1",
            goalId: first?.goalId ?? "goal-smoke",
            action: "Respond to subject",
            dependsOn: [],
            status: "active",
          },
        ],
        rationale: "reference planning stub",
      };
    },
...
export function makeKnowledge(opts: ReferenceBindingOptions): KnowledgeConnectorInterface {
  const sourceId = "custom-pack";
  const corpus: KnowledgePassage[] = [
    {
      sourceId,
      citation: `${sourceId}#1`,
      content: "Reference knowledge passage for smoke tests.",
      score: 1,
      asOf: "2024-06-01",
    },
  ];
